...
#include <bits/stdc++.h>
using namespace std;
// Function which checks if the first m deletions (as per indices in v) can
// still lead to forming p.
bool good(string &t, string &p, vector<int> &v, int m) {
  ...
  for (int i = 0; i < t.size() && idx < p.size(); i++) {
    ...
    if (t[i] == p[idx]) { // Check if the remaining letter matches the current
                          // character in 'p'
